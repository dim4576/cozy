...
- **Переменные с префиксом `NEXT_PUBLIC_`** встраиваются в клиентский код при сборке и доступны в браузере
- **Обычные переменные** (без `NEXT_PUBLIC_`) доступны только на сервере

### После изменения .env файла:

**Если изменили переменные с `NEXT_PUBLIC_` (требуется пересборка):**
...
# 2. Пересоберите проект (обязательно!)
npm run build

# 3. Убедитесь, что статические файлы на месте
if [ ! -d ".next/standalone/.next/static" ]; then
  mkdir -p .next/standalone/.next
  cp -r .next/static .next/standalone/.next/static
fi
...
**Если изменили только серверные переменные (без `NEXT_PUBLIC_`):**

```bash
# Достаточно только перезапуска
pm2 restart cozy-space
```
